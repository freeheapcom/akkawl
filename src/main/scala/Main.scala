...
import com.freeheap.akkawl.message.{FinishCrawling}
import com.freeheap.akkawl.util.{ConfigLoader, Logging}
...
object Main extends Logging {
  ...
  class Listener extends Actor {
    def receive = {
      case FinishCrawling(url, domain) => info(s"${self.path.name} is listening to: $url")
    }
  }
